...
> {
          ...
          Object.keys(filterParams).forEach((key) => {
               ...
               if (key.includes(".")) {
                    ...
                    if (parts.length === 2) {
                         ...
                         (querySome[nestedRelation] as Record<string, unknown>)[nestedField] = this.parseFilterValue(value);
                         (countSome[nestedRelation] as Record<string, unknown>)[nestedField] = this.parseFilterValue(value);
                         return;
                    }
               }
     ...
     fields(): this {
          ...
          if (fieldsParam && typeof fieldsParam === "string") {
               ...
               this.selectFields = {};
               fieldsArray.forEach((field) => { if (this.selectFields) this.selectFields[field] = true; });
     ...
     dynamicInclude(includeConfig: Record<string, unknown>, defaultInclude?: string[]): this {
          ...
          defaultInclude?.forEach((field) => { if (includeConfig[field]) result[field] = includeConfig[field]; });
          const includeParam = this.queryParams.include as string | undefined;
          if (includeParam && typeof includeParam === "string") {
               const requestedRelations = includeParam.split(",").map((r) => r.trim());
               requestedRelations.forEach((relation) => { if (includeConfig[relation]) result[relation] = includeConfig[relation]; });
          }
     ...
     async execute(): Promise<IQueryResult<T>> {
          ...
          return { data: data as T[], meta: { page: this.page, limit: this.limit, total, totalPages } };
     }
     ...
     private parseRangeFilter(value: Record<string, string | number>): PrismaNumberFilter | PrismaStringFilter | Record<string, unknown> {
          ...
          Object.keys(value).forEach((operator) => {
               ...
               switch (operator) {
                    case "lt": case "lte": case "gt": case "gte": case "equals": case "not": case "contains": case "startsWith": case "endsWith":
                         rangeQuery[operator] = parsedValue; break;
                    case "in": case "notIn":
                         rangeQuery[operator] = Array.isArray(operatorValue) ? operatorValue : [parsedValue]; break;
                    default: break;
               }
          });
